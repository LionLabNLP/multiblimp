...
if __name__=="__main__":
    ...
    parser.add_argument("--target_id", default=None,
                        help="Override the output dir name (decision_trees/<id>, "
                             ...
                             "filename (ovGa). Useful for A/B runs that shouldn't "
    ...
    # Object-verb agreement (polypersonal object indexing) -- see ovNa.py's
    # comment and word_order.prediction_target.obj_agr_target's docstring.
    # Gender is the rarest of the three object-agreement dimensions in the
    # UD languages that mark this at all (e.g. Basque: Gender[erg]/
    # Gender[dat] each occur only a couple dozen times vs. thousands for
    # Number/Person) -- expect low candidate counts even where present.
    ...
    pipeline = Pipeline(target=target,
                        ...
                        inflection_map=swap_gender_obj_any,
                        unimorph_args = {
                            "filter_entries": {
                                "upos": ["V"],
                                },
