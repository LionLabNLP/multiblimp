...
if __name__=="__main__":
    ...
    parser.add_argument("--target_id", default=None,
                        help="Override the output dir name (decision_trees/<id>, "
                             ...
                             "filename (ovPa). Useful for A/B runs that shouldn't "
    ...
    # Person, not Number, is the dominant object-agreement dimension in the
    # UD languages that mark this at all (e.g. Georgian object agreement is
    # almost always realized via Person).
    target = obj_agr_target
    target.head_feats = {"VerbForm": partial(filter_head_feats, exclude="Part")}
    target.swap_feat = "Person"
    ...
    pipeline = Pipeline(target=target,
                        ...
                        inflection_map=swap_person_obj_any,
                        unimorph_args = {
                            "filter_entries": {
                                "upos": ["V"],
                                },
